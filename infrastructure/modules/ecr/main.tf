...
resource null_resource ecr_image {
   ...
   provisioner "local-exec" {
     command = <<EOF
             ...
             cd ../
             docker build -t ${aws_ecr_repository.deploy_repo.repository_url}:${var.ecr_image_tag} -f ${var.docker_image_local_path} .
             docker push ${aws_ecr_repository.deploy_repo.repository_url}:${var.ecr_image_tag}
         EOF
   }
}
...
output "image_uri" {
  value     = "${aws_ecr_repository.deploy_repo.repository_url}:${data.aws_ecr_image.lambda_image.image_tag}"
}