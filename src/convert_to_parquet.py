...
def convert_to_parquet():
    ...
    df_train_all = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/train.csv",
        usecols=[1, 2, 3, 4, 5],
        converters={
    ...
    df_test = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/test.csv",
        usecols=[0, 1, 2, 3, 4],
        dtype={"onpromotion": bool},
        parse_dates=["date"],
        low_memory=False,
    ...
    items = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/items.csv"
    ).set_index("item_nbr")
    save_as_parquet(df=items, filename="items.parquet")
    stores = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/stores.csv"
    ).set_index("store_nbr")
    save_as_parquet(df=stores, filename="stores.parquet")
    holidays = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/holidays_events.csv"
    )
    save_as_parquet(df=holidays, filename="holidays.parquet")
    oil = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/oil.csv"
    )
    save_as_parquet(df=oil, filename="oil.parquet")
    transactions = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/transactions.csv"
    )
    save_as_parquet(df=transactions, filename="transactions.parquet")
    submissions = pd.read_csv(
        "../input/favoritagrocerysalesforecastingextracted/sample_submission.csv"
    )
