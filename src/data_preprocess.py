...
def read_parquet_files():
    df2016_train = pd.read_parquet(
                    '../input/grocery-sales-forecasting-parquet/df2016_train.parquet',engine='pyarrow')
    
    df_items = pd.read_parquet(
                    '../input/grocery-sales-forecasting-parquet/items.parquet',engine='pyarrow')
    df_stores = pd.read_parquet(
                    '../input/grocery-sales-forecasting-parquet/stores.parquet',engine='pyarrow')
...
def feature_engg(df2016_train, df_items, df_stores):
    oe = OrdinalEncoder()
    ...
    df_2017 = df_2017.set_index(["store_nbr", "item_nbr", "date"])[["unit_sales"]].\
...
if __name__ == '__main__':
    ...
    df_2017, promo_2017, df_2017_item, promo_2017_item, df_2017_store_class, df_2017_store_class_index = feature_engg(df2016_train, df_items, df_stores)
